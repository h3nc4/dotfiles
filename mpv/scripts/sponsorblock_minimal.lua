...
--
-- This is a modified version of https://codeberg.org/jouni/mpv_sponsorblock_minimal.git
--
...
function skip_ads(name,pos)
	if not has_buffered or not pos then return end
...
function check_buffered(name, pos)
	if pos and pos >= 1 then
		has_buffered = true
		mp.unobserve_property(check_buffered)
	end
end
	...
	if sponsors.stdout then
		...
		if type(json) == "table" then
			...
			if ranges then
				...
				mp.observe_property("time-pos", "native", check_buffered)
			end
		end
	end
end
